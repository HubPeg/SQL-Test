...
Entwicklung der ETL-Pipelines
• Entwicklung der ETL-Pipelines mit SQL Stored Procedures:
    Die Daten sollen zuerst 1:1 aus dem Quelltabellen bzw. dem «ERP» in die neue Datenbank in das Schema «Staging_<name>» kopiert werden. <- Das wurde zuvor gemacht.
    Anschliessend sollen die Daten von der Staging Schicht in die Star-Schemas geladen werden. <- Dieser Prozess findet mit der Beladung statt.
*/
...
/*
Mit diesem Schritt werden nun die Dimensionen & Faktentabellen befüllt.
SELECT DISTINCT = Keine doppelten Daten, SELECT = Alle Daten.
*/